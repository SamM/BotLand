...
BotLand.prototype.make.bot = function(parent, object){
  ...
  if(this.self.selected_bot){
    ...
    var details_list = this.list(["30%", "70%"], rows, el);

    var queued = this.heading(3, "Queued Messages", el);
    queued.style.marginTop = "1em";

    var queued_messages = bot.queued_messages;

    if(queued_messages.length){
      var heading_list = this.list(["30%", "70%"], [this.text("To"), this.text("Message")], el)
      ...
      var message_list = this.list(["30%", "70%"], rows, el)
      message_list.style.margin = "0 1em 0 1em";
    }else{
      var notice = this.div(el);
      notice.style.margin = "0 1em";
      this.text("There are no queued messages.", notice);
    }

    var say_heading = this.heading(3, "Say Message", el);
    ...
    var send_button = this.button("Queue Message", function(e){
      ...
      make.self.render();
    }, this.div(el).set("style.marginTop", "1em"));
    ...
    var heading_list = this.list(["50%", "40%"], [this.text("Name"), this.text("Model")], el);
    heading_list.style.margin = "0 1em 0 1em";
    heading_list.style.fontWeight = "bold";

    var row_list = this.list(["50%", "40%"], rows, el);
...
BotLand.prototype.make.land = function(parent, object){
  ...
  var messages_heading = this.heading(3, "Messages", el);
  messages_heading.style.margin = "0 0 0.5em 0.75em";
  ...
  if(messages.length){
    ...
    var heading_list = this.list(["20%", "20%", "50%"], [this.text("From"), this.text("To"), this.text("Message")], el)
    heading_list.style.margin = "0 1em 0 1em";
    heading_list.style.fontWeight = "bold";

    var message_list = this.list(["20%", "20%", "50%"], rows, el)
    message_list.style.margin = "0 1em 0 1em";
  }else{
    var notice = this.div(el);
